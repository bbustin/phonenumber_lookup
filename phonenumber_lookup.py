...
def parse_single_number(phone_number,  locale, language=DEFAULT_LANGUAGE, **kwargs):
	...
	if len(results) == 0 or len(results) > 1:
		final_result = {'raw_input': phone_number, 'comment': 'Needs manual review: Possible match for {} locales ({})'.format(len(results), ", ".join([result['assumed_local_locale'] for result in results]))}
	else:
...
if __name__ == "__main__":
    ...
    group_locale.add_argument("-all_locales", dest='locale', action="store_const", const=LOCALES)
